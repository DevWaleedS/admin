		...
		<div className='absolute pl-[140px] pr-5 py-[43px]  top-0 right-0  z-10  pb-36  w-full otlobha_acadmic' style={{ backgroundColor: '#fafafa' }}>
			...
			<div className='flex justify-between items-center mb-2'>
				<div className='flex'>
					<div className={`flex items-center gap-2 `}>
						...
						<h2 style={{ fontSize: '18px' }} className='font-medium ml-4'>
							{' '}
							الرئيسية{' '}
						</h2>
					</div>
			...
			<div className='flex flex-col gap-5 mt-[42px] pr-[78px]'>
				<div className='flex flex-col gap-[10px]'>
					<label style={{ color: '#011723', fontSize: '18px' }}>اسم الدورة التدريبية</label>
					<input className='w-full outline-none shadow-sm rounded-lg p-4' style={{ backgroundColor: '#F4F5F7', border: '1px solid #67747B33' }} type='text' name='name' />
				</div>
				<div className='flex flex-col gap-[10px]'>
					<label style={{ color: '#011723', fontSize: '18px' }}>الوصف</label>
				...
				<div className='flex flex-col gap-[10px]'>
					...
					<div style={{ backgroundColor: '#F4F5F7', border: '1px solid #67747B33' }} className='flex flex-row items-center flex-wrap gap-6 p-5 rounded-lg'>
						{tags.map((tag, index) => (
							<span key={index} style={{ fontSize: '18px', color: '#67747B', backgroundColor: '#EBEBEB', borderRadius: '18px' }} className='px-3 py-2'>
								{tag}
							</span>
						))}
					</div>
						...
						{({ onImageUpload, dragProps }) => (
							...
							<div
								...
								style={{
									width: '100%',
									padding: '8px',
									border: images[0] || '1px dashed #02466A',
									borderRadius: '10px',
				...
				<div className='flex flex-col gap-[10px]'>
					...
					<div className='flex flex-row items-center justify-between rounded-md p-4' style={{ backgroundColor: '#F4F5F7', border: '1px solid #67747B33' }}>
				...
				<div className='flex mt-10 gap-4'>
					...
					<Button style={{ borderColor: '#02466A' }} textStyle={{ color: '#02466A', fontSize: '20px' }} className={'flex-1'} type={'outline'} onClick={cancel}>
						إلغاء
					</Button>
				</div>
			</div>
