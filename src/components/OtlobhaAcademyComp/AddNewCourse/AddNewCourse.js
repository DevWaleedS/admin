		...
		<div className='absolute h-full md:pl-[140px] md:pr-5 md:py-[43px] top-0 right-0 z-30 md:pb-36 w-full md:bg-[#fafafa] bg-[#FFFFFF] otlobha_acadmic'>
			...
			<div className='flex justify-between items-center mb-2 md:p-0 px-4'>
				<div className='flex'>
					...
					{editData ? (
						<h3 className='md:text-[18px] text-[15px] font-medium' style={{ color: '#67747B' }}>
							/ تعديل دورة تدريبية
						</h3>
					) : (
						<h3 className='md:text-[18px] text-[15px] font-medium' style={{ color: '#67747B' }}>
			...
			<div className='flex flex-col gap-5 mt-[42px] md:pr-[78px] p-4 pb-28 '>
				...
				<div className='flex flex-col gap-[10px]'>
					<h2 className='md:text-[18px] text-[16px]' style={{ color: '#011723' }}>
						محاور الدورة TAGS
					</h2>
					<div style={{ backgroundColor: '#F4F5F7', border: '1px solid #67747B33' }} className={`${styles.tags} flex flex-row items-center flex-wrap rounded-lg`}>
						<TagsInput value={tagsSelected} onChange={setTagsSelected} name='tags' placeHolder='ادخل محاور الدورة TAGS' />
					</div>
				</div>
				<div className='flex flex-col gap-3'>
					<div className='flex md:flex-row flex-col gap-1'>
						...
						<p className='md:text-[16px] text-[14px]' style={{ color: '#ADB5B9' }}>
				...
				{unitDetails.length !== 0 && (
					<div className='mb-[80px] mt[33px] flex flex-col gap-4'>
						<h6 className='md:text-[24px] text-[20px]' style={{ fontWeight: '500', color: '#000000' }}>
							دروس الدورة
						</h6>
						<div>
								...
								>
									...
									>
										<div className='flex flex-row items-center gap-2'>
											<h6 className='md:text-[18px] text-[16px]' style={{ fontWeight: '500', color: '#000000' }}>
												{item?.title}
											</h6>
											<span className='md:text-[18px] text-[16px]' style={{ color: '#67747B' }}>
												({item?.videos?.length + item?.documents?.length} دروس)
											</span>
											<span className='md:text-[18px] text-[16px]' style={{ color: '#67747B' }}>
												(25 دقيقة)
											</span>
									...
									<AccordionDetails style={{ backgroundColor: '#F4F5F7' }} className='flex flex-col gap-5 absolute z-10 left-0 top-[55px] w-full p-5'>
										{item?.videos?.map((video, index) => (
											<div key={index} className='flex flex-row items-center justify-between'>
												<div className='flex flex-row items-center'>
													<PlayVideo />
													<h6 style={{ fontWeight: '500', color: '#011723' }} className='md:text-[20px] text-[18px] mr-[20px] ml-[30px]'>
														{video?.name}
													</h6>
													<span className='md:text-[18px] text-[16px]' style={{ color: '#011723' }}>
														10:05
													</span>
												</div>
												<DeleteIcon
										...
										{Array.from(item?.documents || [])?.map((file, index) => (
											<div key={index} className='flex flex-row items-center justify-between'>
												<div className='flex flex-row items-center'>
													<PDFIcon />
													<h6 style={{ fontWeight: '500', color: '#0077FF' }} className='md:text-[20px] text-[18px] mr-[20px] ml-[30px]'>
														{file?.name}
													</h6>
												</div>
												<h6 className='md:text-[18px] text-[16px]' style={{ color: '#0077FF', cursor: 'pointer' }}>
													تحميل
												</h6>
											</div>
										))}
										...
										{item?.videos?.map((video, index) => (
											<div key={index} className='flex flex-row items-center justify-between'>
												<div className='flex flex-row items-center'>
													<PlayVideo />
													<h6
														style={{ maxWidth: '600px', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis', fontWeight: '500', color: '#011723' }}
						...
						{({ onImageUpload, dragProps }) => (
							...
							<div
								...
								style={{
									...
									border: images[0] || editData ? 'none' : '1px dashed #02466A',
									borderRadius: '10px',
				...
				{editData && (
					<div className='flex flex-col gap-[10px]'>
						<label className='md:text-[20px] text-[18px]' style={{ color: '#02466A' }}>
							رابط الدورة
							<span className='md:text-[16px] text-[14px]' style={{ color: '#02466A' }}>
								{' '}
								(تلقائي){' '}
							</span>
						</label>
						<div className='flex flex-row items-center justify-between rounded-md gap-3 p-4' style={{ backgroundColor: '#F4F5F7', border: '1px solid #67747B33' }}>
