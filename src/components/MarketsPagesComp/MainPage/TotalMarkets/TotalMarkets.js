...
const TotalMarkets = () => {
	const [storeCount, setStoreCount] = React.useState('');


  // useEffect(() => {
  //  fetch('https://backend.atlbha.com/api/Admin/activity')
	// 			.then((res) => res.json())
			...
			<div className='flex-1 w-64 rounded-lg flex md:flex-row flex-col items-center p-4' style={{ backgroundColor: '#B6BE34' }}>
				...
				<div className='flex flex-1 h-full flex-col justify-between pt-4'>
					...
					<div className='flex flex-col md:items-start items-center'>
						<h2 className='text-slate-50 font-semibold md:text-[18px] text-[20px]'>{storeCount}K</h2>
						<h6 className='text-slate-50  text-xs mt-3 text-center'>20% + خلال هذا الشهر</h6>
					</div>
				</div>
			</div>
