		...
		<div className='flex-1 shadow-lg rounded-lg overflow-hidden'>
			...
			<div className='bg-white'>
				{latestStores?.map((store,index) => {
					return (
						<a key={index} href={store?.domain} target='_blank' rel='noreferrer' className='flex justify-between md:p-4 p-2 md:h-24 h-16 shadow-[0px_3px_6px_#02466A0F] bg-[#F7FCFF] mb-2'>
							<div className='flex flex-row md:gap-16 gap-3'>
								<div className='flex flex-col'>
									<img className='md:h-14 h-[30px] md:w-14 w-[30px] object-cover  rounded-full' src={store?.icon} alt={store?.store_name} />
								</div>
