...
const images = [{ id: 1, url: Cam1 }, { id: 2, url: Cam2 }, { id: 3, url: Cam3 }];
          ...
          <div className="flex md:flex-row flex-col items-center gap-[18px]">
            ...
            <div className="flex md:flex-col flex-row gap-4">
              {images?.map((item, index) => (
                <div key={index} className="flex flex-col items-center justify-center" style={{ width: '80px', height: '65px', border: '1px solid #EEEEEE' }}>
                  <img className="w-full" src={item?.url} alt="small-img" onClick={()=>setImageUrl(item?.url)}/>
                </div>
              ))}
            </div>
          </div>
